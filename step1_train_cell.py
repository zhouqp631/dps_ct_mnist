...
def parse_args():
    ...
    # parser.add_argument('--ckpt', type=str, help='define checkpoint path', default=f'results\cell_steps_00011009.pt')
    parser.add_argument('--n_samples', type=int, help='define sampling amounts after every epoch trained', default=36)
    parser.add_argument('--model_base_dim', type=int, help='base dim of Unet', default=64*2)
...
def main(args):
    ...
    train_dataloader, test_dataloader = create_inverse_scatter_dataloaders(batch_size=args.batch_size, image_size=32)
    ...
    for i in range(args.epochs+1):
        ...
        if i>=10 and i % 10 == 0:
            ...
            torch.save(ckpt, "results/cell_steps_{:0>8}.pt".format(global_steps))

    model.eval()
    samples = model.sampling(args.n_samples, clipped_reverse_diffusion=not args.no_clip, device=device)
    save_image(samples, "results/cell_steps_{:0>8}.png".format(global_steps), nrow=int(math.sqrt(args.n_samples)))
