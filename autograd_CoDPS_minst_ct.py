...
for _ in range(3):
    ...
    for i in tqdm(range(N-1,0,-1)):
        ...
        A_x0 = A @ x0_hat_vec
        # Sigma_y 为 cov
        Sigma_y = (sigma_y ** 2) * torch.eye(A.shape[0], device=device) + sigma0t_sq * (A @ A.T)
        residual = y - A_x0
        solved = torch.linalg.solve(Sigma_y, residual)
        ...
        grad = torch.autograd.grad(loglike_neg, x_rec)[0]
    ...
    plt.plot(t_values, mse_list, label=f'sigma_y={sigma_y}')
