...
from utils_data import create_mnist_dataloaders,create_inverse_scatter_dataloaders
...
from inverse_scatter import construct_parameters, conv2D, conv2DAdj
from inverse_scatter import InverseScatter

inverse = InverseScatter(Nx=64, Ny=64, numTrans=60, numRec=60, wave=6, sensorRadius=1.6)
inverse.compute_svd()
A = inverse.A
# 读取 MNIST 图像作为散射体 f
image_size = 64
train_dataloader, test_dataloader = create_inverse_scatter_dataloaders(batch_size=1, image_size=image_size)
image = next(iter(test_dataloader))
x = image[0].to(device)

y_noise_free = A @ x.reshape(-1,1)  # shape: [7200,1]
sigma_y = 0.1 * torch.max(torch.abs(y_noise_free))
y = y_noise_free + sigma_y * torch.randn_like(y_noise_free)
x_fbp = torch
# # ---Figure----
...
checkpoint = torch.load(f"results/cell_steps_00053519.pt", map_location=device,weights_only=True)
...
for _ in range(3):
    ...
    for i in tqdm(range(N-1,0,-1)):
        ...
        # reverse_std = torch.sqrt(reverse_std)
        ...
        x_iminus1 = (torch.sqrt(alphas[i])*(1-alphas_cumprod[i-1])/(1-alphas_cumprod[i])*x_rec +
                     torch.sqrt(alphas_cumprod[i-1])*betas[i]/(1-alphas_cumprod[i])*x0_hat +
                     reverse_std*z)

        # line 7 of Algorithm 1: update x with gradient scaling
        norm = torch.linalg.norm(y - A@x0_hat.reshape(-1,1))
        norm_grad = torch.autograd.grad(norm, x_rec)[0]

        # Adaptive gradient scaling
        grad_norm = torch.norm(norm_grad)
        if grad_norm > 1.0:
            norm_grad = norm_grad / grad_norm

        # zeta_i = zeta / (norm.item() + 1e-6)  # Add small epsilon to prevent division by zero
        x_rec = x_iminus1 - torch.sqrt(alphas_cumprod[i]) * norm_grad
